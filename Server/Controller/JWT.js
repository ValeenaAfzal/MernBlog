...
export const authenticateToken = (request, response, next) => {
    ...
    jwt.verify(token, process.env.ASecretKey, (error, user) => {
        if (error) {
            return response.status(403).json({ msg: 'invalid token' })
        }

        request.user = user;
        next();//pass middle ware to create post
    })
}
...
export const createNewToken = async (request, response) => {
    ...
    jwt.verify(token.token, process.env.RSecretKey, (error, user) => {
        if (error) {
            response.status(500).json({ msg: 'invalid refresh token'});
        }
        const accessToken = jwt.sign(user, process.env.ASecretKey, { expiresIn: '15m'});
